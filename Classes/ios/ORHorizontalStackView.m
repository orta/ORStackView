//
//  ORHorizontalStackView.m
//  ORStackView
//
//  Created by Dzianis Lebedzeu on 4/23/15.
//
//

#import "ORHorizontalStackView.h"
#import <FLKAutoLayout/UIView+FLKAutoLayout.h>
#import "ORStackView+Private.h"

@interface ORHorizontalStackView()
...
@implementation ORHorizontalStackView
...
- (void)updateConstraints
{
    ...
    for (StackView *stackView in self.viewStack) {
        ...
        if (index == 0) {
            stackView.topConstraint = [[view alignLeadingEdgeWithView:self predicate:predicate] lastObject];
        } else {
            UIView *viewAbove = [self.viewStack[index - 1] view];
            stackView.topConstraint = [[view constrainLeadingSpaceToView:viewAbove predicate:predicate] lastObject];
        }
    }
    
    if (self.trailingMarginWidth != NSNotFound) {
        [self removeConstraint:self.trailingConstraint];
        UIView *lastView = self.lastView;
        if (self.lastView) {
            NSString *constraint = [NSString stringWithFormat:@"%0.0f", self.trailingMarginWidth];
            self.trailingConstraint = [[self alignTrailingEdgeWithView:lastView predicate:constraint] lastObject];
        }
    }
...
- (void)_insertSubview:(UIView *)view atIndex:(NSInteger)index withTopMargin:(CGFloat)topMargin centered:(BOOL)centered sideMargin:(CGFloat)sideMargin
{
  ...
  //  stackView.topConstraint = [constraints firstObject];
    ...
    if (centered) {
        //TODO: handle topLayoutGuide here
